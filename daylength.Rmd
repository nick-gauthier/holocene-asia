...
library(insol)
library(chillR)
bbox <- st_bbox(c(xmin = 100, xmax = 150, ymin = -15, ymax = 45), crs = 4326)
...
clim <- read_stars(c('~/Downloads/CHELSA_gdd10_1981-2010_V.2.1.tif',
           ...
           '~/Downloads/CHELSA_fcf_1981-2010_V.2.1.tif')) %>%
    st_crop(bbox) %>%
    #st_as_stars() %>%  
    #st_warp(ref, method = 'average', use_gdal = TRUE) %>%
    setNames(c('gdd10', 'gddfgd10', 'gddfgd', 'fcf'))
```
...
```{r}
countries <- rnaturalearth::ne_coastline(returnclass = 'sf', 
                                         scale = 'small') %>%
  st_crop(east_asia)
plot(countries)
ggplot() +
