...
An analysis of present and past temperature and precipitation variability across east and southeast using the [ERA5-Land reanalysis](https://www.ecmwf.int/en/era5-land) and [CHELSA-TraCE downscaled climate data](https://chelsa-climate.org). 
...
Now we'll use downscaled outputs from the TraCE-21ka simulation, a transient simulation of the Last Glacial Maximum to present using CCSM3. The primary forcings are insolation, meltwater, green house gasses, and land cover change.

```{r trace-prc-import, cache = TRUE, include=FALSE, eval = FALSE}
# load and reproject the CHELSA-TraCE data. not run by default becuase its slow!
...
p2 <- get_patterns(trace['prc'], k = 5, scale = TRUE, rotate = TRUE)
```
...
plot_eofs(p1) +
  ...
  ggtitle('Leading modes of century-scale precipitation variability', 'CHELSA-TraCE, 12ka to Present')
```
...
PC3 (rotated) is GHG forcing, PC1 is some mix of Indonesian Throughlfow and insolation warming. PC2 almost looks like the monsoon/rainfall signal --  or better yet the ice sheets
...
ggplot() +
 ...
 scale_fill_distiller(palette = 'BrBG', limits = c(-1400, 1400), direction = 1) +
  geom_sf(data = coasts) +
  labs(title = 'Precipitation change, 12k to Present')
```
...
So we have early and late Holocene equilibria. General warming trend, punctuated by southward shift of the monsoon.
...
1. Connect climate variables to (rice) crop growth.
2. Seasonal reconstructions.
3. Expand to Mid Holocene and Last Millennium PMIP simulations.
