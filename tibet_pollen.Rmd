...
```{r}
plot(mat_gdd0);plot(mat_gdd5);plot(mat_gdd10)
```
...
```{r}
pastgdd <- predict(gdd_mat, dat$fossil_pollen / 100, k = 4)
...
```{r}
gdd0_wapls <- rioja::WAPLS(dat$modern_pollen / 100, as_tibble(pts)$GDD0, 
                           npls = 10, 
                           standx = FALSE) %>% 
  crossval(gdd0_wapls, cv.method = 'lgo', ngroups = 5)

screeplot(gdd0_wapls)
plot(gdd0_wapls)
```


```{r}
gdd_wa <- rioja::WA(dat$modern_pollen / 100, as_tibble(pts)$GDD10) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
plot(gdd_wa)
```
