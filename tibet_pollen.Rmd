...
read_stars('/vsigzip//Users/nick/Downloads/CMFD/temp_CMFD_V0106_B-01_01dy_010deg_197901-197912.nc.gz')
```
...
```{r}
pts <- st_extract(clim, at = ica_scores) # bilinear = TRUE causes this to fail for some reason
dat_gam <- ica_scores %>%
  mutate(mat = pts$MAT,
         tap = pts$TAP,
         gdd0 = pts$GDD0,
...
```{r}
mat_gdd0 <- rioja::MAT(dat$modern_pollen / 100, as_tibble(pts)$GDD0, lean = FALSE) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
mat_gdd5 <- rioja::MAT(dat$modern_pollen / 100, as_tibble(pts)$GDD5, lean = FALSE) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
mat_gdd10 <- rioja::MAT(dat$modern_pollen / 100, as_tibble(pts)$GDD10, lean = FALSE) %>%
  crossval(cv.method = 'lgo', ngroups = 5)

screeplot(mat_gdd0);screeplot(mat_gdd5);screeplot(mat_gdd10)
```

```{r}
plot(mat_gdd0, k = 3, wMean = TRUE);plot(mat_gdd5,k = 3, wMean = TRUE);plot(mat_gdd10,k = 3, wMean = TRUE)
```
...
```{r}
gdd0_pcr <- pcr(dat$modern_pollen / 100, as_tibble(pts)$GDD0, ncomp = 10, tranFun = Hellinger)
gdd5_pcr <- pcr(dat$modern_pollen / 100, as_tibble(pts)$GDD5, ncomp = 10, tranFun = Hellinger)
gdd10_pcr <- pcr(dat$modern_pollen / 100, as_tibble(pts)$GDD10, ncomp = 10, tranFun = Hellinger)

screeplot(gdd0_pcr);screeplot(gdd5_pcr);screeplot(gdd10_pcr)
```

```{r}
gdd10_wapls <- rioja::WAPLS(sqrt(dat$modern_pollen / 100), as_tibble(pts)$GDD10, 
                           npls = 10,
                           standx = FALSE) %>% 
  crossval(cv.method = 'lgo', ngroups = 5)
summary(gdd10_wapls)
screeplot(gdd10_wapls)
plot(gdd10_wapls, npls = 4)
```


```{r}
gdd10_wa <- rioja::WA(dat$modern_pollen / 100, as_tibble(pts)$GDD10) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
...
```{r}
gdd_mlrc <- rioja::MLRC(dat$modern_pollen / 100, as_tibble(pts)$GDD10) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
plot(gdd_mlrc)
```

```{r}
gdd_LWR <- rioja::LWR(dat$modern_pollen / 100, as_tibble(pts)$GDD10) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
plot(gdd_LWR)
...
```
```{r}
gdd_LWR_sqrt <- rioja::LWR(sqrt(dat$modern_pollen / 100), as_tibble(pts)$GDD10) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
summary(gdd_LWR_sqrt);summary(gdd_LWR)

plot(gdd_LWR$x, gdd_LWR$predicted[,1])
plot(gdd_LWR_sqrt$x, gdd_LWR_sqrt$predicted[,1])

```

```{r}
gdd_IKFA <- rioja::IKFA(dat$modern_pollen / 100, as_tibble(pts)$GDD10, nFact = 10, IsPoly = TRUE) %>%
  crossval(cv.method = 'lgo', ngroups = 5)
...
china_hex <- china %>% 
  ...
  mutate(x = st_coordinates(st_centroid(.))[,1],
         y = st_coordinates(st_centroid(.))[,2]) %>%
  filter(st_intersects(geometry, china, sparse = FALSE)) %>%
  mutate(id = 1:n())
```
