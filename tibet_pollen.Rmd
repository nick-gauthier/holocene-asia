...
```
```{r}
t(ica$A)[,1, drop = FALSE] %>% as.data.frame(row.names = names(modern_pollen)) %>% arrange(-abs(V1)) %>% slice_head(n = 5)
t(ica$A)[,2, drop = FALSE] %>% as.data.frame(row.names = names(modern_pollen))%>% arrange(-abs(V1)) %>% slice_head(n = 5)
t(ica$A)[,3, drop = FALSE] %>% as.data.frame(row.names = names(modern_pollen))%>% arrange(-abs(V1)) %>% slice_head(n = 5)
t(ica$A)[,4, drop = FALSE] %>% as.data.frame(row.names = names(modern_pollen)) %>% arrange(-abs(V1)) %>% slice_head(n = 5)
t(ica$A)[,5, drop = FALSE] %>% as.data.frame(row.names = names(modern_pollen))%>% arrange(-abs(V1)) %>% slice_head(n = 5)
```
...
mat_gdd0_hblock <- MAT(dat$modern_pollen[,-298] / 100, as_tibble(pts)$gdd0 , lean = FALSE) %>%
  crossval(cv.method = 'h-block', h.cutoff = units::set_units(100000, m), h.dist = dist_mat)

gdd_wapls <- WAPLS(sqrt(dat$modern_pollen[,-298] / 100),as_tibble(pts)$gdd0, npls = 10, 
                   standx = FALSE) %>% 
...
```{r}
screeplot(mat_gdd0_hblock)
screeplot(gdd_wapls)
```

```{r}
plot(gdd_wapls, npls = 2)
plot(mat_gdd0_hblock, k = 3, wMean = TRUE)
```
...
vimp <- randomPTF(dat$modern_pollen[,-298] / 100, as_tibble(pts)$gdd0 , fun=WAPLS, npls = 2, 
                   standx = FALSE,ncol = 2, do.parallel=TRUE, nTF = 10000)
